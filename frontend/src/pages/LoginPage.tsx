...
import { ArrowRight, Mail, Lock, Sparkles, CheckCircle2, AlertCircle } from 'lucide-react';
...
export const LoginPage: React.FC<LoginPageProps> = ({ onNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const success = await loginWithEmail(cleanEmail, password);
      if (success) {
        onNavigate('/home');
      }
    } catch {
      setValidationError(
        isHindi ? 'लॉगिन विफल रहा, पुनः प्रयास करें' : 'Login failed, please try again'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="w-full min-h-[80vh] flex items-center justify-center px-4 py-12 animate-fade-in">
      <div className="w-full max-w-md p-8 sm:p-9 rounded-3xl bg-surface border border-border shadow-md focus-card">
        {/* Brand header */}
        <div className="text-center mb-8">
          <div className="flex justify-center mb-3.5">
            <FactSetuLogo size={48} />
          </div>
          ...
          <p className="text-xs sm:text-sm text-text-secondary mt-1.5">
            {isHindi ? 'सत्य और प्रामाणिक सूचना का सेतु' : 'Bridge to Trustworthy & Verified Information'}
          </p>
        </div>
        ...
        <div className="space-y-3 mb-6">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span>{isHindi ? 'Google से लॉगिन करें' : 'Continue with Google'}</span>
            </div>
        ...
        {/* Email / Password Form with Gmail & Length Constraints */}
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full pl-10 pr-4 py-3 rounded-2xl bg-surface-secondary border border-border focus:border-primary focus:ring-2 focus:ring-primary/20 text-xs sm:text-sm font-medium text-text-primary outline-none transition-all"
              />
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 rounded-2xl bg-primary text-white font-bold text-xs sm:text-sm shadow-xs hover:bg-primary-hover active:scale-[0.98] transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
            <span>{loading ? 'Verifying...' : t('auth_btn_login')}</span>
            <ArrowRight size={15} />
          </button>
        </form>

        {/* Guest Access Option */}
        <div className="mt-5 pt-5 border-t border-border/70 text-center">
          <button
            type="button"
            onClick={handleContinueAsGuest}
            className="w-full py-2.5 px-4 rounded-2xl bg-surface-secondary/70 hover:bg-surface-secondary border border-border text-xs font-bold text-text-primary flex items-center justify-center gap-2 transition-all cursor-pointer group"
          >
            <Sparkles size={14} className="text-primary group-hover:rotate-12 transition-transform" />
            <span>{isHindi ? 'अतिथि के रूप में जारी रखें (Explore as Guest)' : 'Continue as Guest (No Login Required)'}</span>
          </button>
        </div>
        ...
        <div className="mt-4 flex items-center justify-center gap-1.5 text-[11px] text-text-muted text-center">
          <CheckCircle2 size={12} className="text-emerald-500 shrink-0" />
          <span>{t('auth_anonymous_note')}</span>
        </div>
      </div>
